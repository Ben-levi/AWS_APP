...
EOF
                            ...
                            # Configure MySQL with transaction isolation level
                            echo "Creating MySQL ConfigMap with READ-COMMITTED transaction isolation..."
                            cat <<EOF | kubectl apply -f - -n ${env.MYSQL_NS}
apiVersion: v1
kind: ConfigMap
metadata:
  name: mysql-config
                ...
                sshagent([env.SSH_CRED_ID]) {
                    sh """
                        ...
                        ssh -o StrictHostKeyChecking=no ${env.SSH_USER}@${env.EC2_HOST} '
                            ...
                            # Configure application to use single replica for file storage
                            echo "Setting application deployment to use single replica..."
                            kubectl scale deployment \$DEPLOYMENT_NAME -n ${env.APP_NS} --replicas=1
                            
                            # Patch the deployment to use emptyDir instead of PVC for uploads
                            echo "Patching deployment to use emptyDir for uploads..."
                            kubectl patch deployment \$DEPLOYMENT_NAME -n ${env.APP_NS} --type=json -p=\'[{"op":"replace","path":"/spec/template/spec/volumes/10","value":{"name":"uploads","emptyDir":{}}}]\'
                        '
                    """
                }
            }
                    ...
                    sh """
                        ...
                        ssh -o StrictHostKeyChecking=no ${env.SSH_USER}@${env.EC2_HOST} '
                            ...
                            if [ -n "$MYSQL_POD" ]; then
                                ...
                                echo "Creating tasks table with proper schema:"
                                kubectl exec -it $MYSQL_POD -n ${env.MYSQL_NS} -- mysql -u root -ppassword -e "
                                USE todos;
                                ...
                                CREATE TABLE tasks (
                                  ...
                                  status ENUM('New', 'In Progress', 'Completed', 'On Hold') NOT NULL DEFAULT 'New',
                                  person VARCHAR(100),
                                  description TEXT,
                                  photo VARCHAR(255),
                                ...
                                kubectl exec -it $MYSQL_POD -n ${env.MYSQL_NS} -- mysql -u root -ppassword -e "USE todos; INSERT INTO tasks (title, priority, status, person, description) VALUES ('Test Task', 'High', 'New', 'Admin', 'This is a test task');"
